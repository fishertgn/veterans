...
def generar(d0,d1,outdir,test=False,tipus='resultats'):
    ff=ffmpeg()
    rows=P.partits(d0,d1,jugats=(False if tipus=='proxims' else None))
    if tipus=='proxims':
        if not rows: return []
    elif test:
        random.seed(7)
        for i,r in enumerate(rows): r['hs'],r['as_']=random.choice([0,1,1,2,2,3,4]),random.choice([0,1,1,2,2,3]); r['status']=1 if i in (2,9) else 5
    if tipus!='proxims' and not any(r['status']==5 for r in rows): return []
    ...
    for dia in dies:
        ...
        for pag,tros in enumerate([rs[i:i+12] for i in range(0,len(rs),12)],1):
            base=(P.html_story if tipus=='proxims' else R.html_story)(et,tros,1,1)
    ...
    mp4=P.nom_fitxer(outdir,'PROXIMS PARTITS' if tipus=='proxims' else 'RESULTATS',[r['comp'] for r in rows],dies[0],dies[-1],'REEL')[:-4]+'.mp4'
    ...
    return [(f"REEL · {'Pròxims partits' if tipus=='proxims' else 'Resultats'} · {sum(d for _,d in llista):.0f} s",mp4)]
if __name__=='__main__':
    # reel.py resultats AAAA-MM-DD AAAA-MM-DD [--test] [--send]   |   reel.py proxims [--send]  (cap de setmana vinent)
    import publicar as U
    tipus=sys.argv[1]; out=os.path.join(V,'out'); os.makedirs(out,exist_ok=True)
    if tipus=='proxims': d0,d1=U.cap_de_setmana(P.avui_madrid())
    else: d0=datetime.date.fromisoformat(sys.argv[2]); d1=datetime.date.fromisoformat(sys.argv[3])
    if tipus=='proxims' and not P.partits(d0,d1,jugats=False): d0,d1=d0+datetime.timedelta(days=7),d1+datetime.timedelta(days=7)
    res=generar(d0,d1,out,test='--test' in sys.argv,tipus=tipus)
    print(tipus,d0,d1,'->',res or 'res a generar')
    if '--send' in sys.argv:
        for e,p in res: print(U.envia_fitxer(p,e,'reels'))
